...
class WasteTracker:
    def __init__(self, trash_model_path, evidence_path, distance_threshold, pair_consistency_threshold, max_inactive_frames, 
        ...
        self.yolo_model = YOLO("yolov8m.pt")  # YOLOv8 for humans and vehicles
        self.trash_model = YOLO(trash_model_path)  # Your custom model for trash detection
    ...
    def save_evidence(self, entity_id, frame_buffer, current_frame, entity_type):
        # Create a subfolder based on entity type and ID
        ...
        print(f"Saving evidence for {entity_type.capitalize()} ID {entity_id} in {entity_folder}")
        ...
        for idx, frame in enumerate(frame_buffer):
            ...
            cv2.putText(frame_clone, f"{entity_type.capitalize()} ID {entity_id}", (x1, y1 - 10), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            cv2.imwrite(os.path.join(entity_folder, f"frame_{idx:03d}.jpg"), frame_clone)

        current_frame_clone = current_frame.copy()
        x1, y1, x2, y2 = map(int, self.tracking_data[entity_id]['bbox'])
        cv2.rectangle(current_frame_clone, (x1, y1), (x2, y2), (0, 0, 255), 2)
        cv2.putText(current_frame_clone, f"{entity_type.capitalize()} ID {entity_id}", (x1, y1 - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        cv2.imwrite(os.path.join(entity_folder, "final_frame.jpg"), current_frame_clone)
    ...
    def process_video(self, video_path):
        ...
        while cap.isOpened():
            ...
            # Visualization remains the same as your original code
            for det in detections:
...
if __name__ == "__main__":
    ...
    video_path = r"C:\Users\DELL\Desktop\trash\veh_pavi\vehidet\video2.mp4"
